...
from core.problem_solver import execute_and_verify, verify_path_exists
...
def create_file(path: str, name: str = "", content: str = "") -> str:
    try:
        ...
        outcome = execute_and_verify(
            action_fn=_write,
            verify_fn=lambda: verify_path_exists(target),
            max_retries=1,
        )
        if not outcome.verified:
            return f"Could not confirm file was created: {target}"
...
def create_folder(path: str, name: str = "") -> str:
    try:
        ...
        outcome = execute_and_verify(
            action_fn=lambda: target.mkdir(parents=True, exist_ok=True),
            verify_fn=lambda: verify_path_exists(target),
            max_retries=1,
        )
        if not outcome.verified:
            return f"Could not confirm folder was created: {target}"
